...
import { Clock3, FileText, LineChart, MapPin, Search, Sparkles, GitCompareArrows } from 'lucide-react'

export type ThinkingAnimationType = 'thinking' | 'reading' | 'drawing' | 'mapping' | 'finding' | 'eta' | 'comparing'
...
export function ThinkingAnimation({ type = 'thinking' }: { type?: ThinkingAnimationType }) {
  ...
  if (type === 'mapping') return <div className="animation-visual map-animation" aria-hidden="true"><MapPin size={25} /><span className="map-route route-one" /><span className="map-route route-two" /><span className="map-pulse" /></div>
  if (type === 'finding') return <div className="animation-visual finding-animation" aria-hidden="true"><Search size={27} /><span className="finding-scan" /><span className="finding-dot" /></div>
